...
if (isset($_GET['token'])) {
    ...
    if (isset($stmt['ID'])) {
        $id_user = $stmt['ID'];
        $result = '{"pokupki":[';
        $sql = sprintf('SELECT t.`ID`, t.`TITLE`, t.`DESCRIPTION`, t.`PRICE`, k.`COUNT`, kat.`NAME` FROM `korzina` AS k JOIN `tovary` AS t ON k.`ID_TOVAR` = t.`ID` JOIN `kategorii` AS kat ON t.`ID_CAT` = kat.`ID` WHERE `ID_USER` = %d', $id_user);
        $stmt = $db->query($sql);
